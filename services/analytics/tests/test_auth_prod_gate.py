"""A1-08/A1-10 regression: analytics prod fail-closed gates.
...
def test_validate_auth_config_prod_requires_audience(monkeypatch):
    ...
    # audience configured -> allowed
    monkeypatch.setenv("KEYCLOAK_AUDIENCE", "analytics")
    auth_mod.validate_auth_config("keycloak", "prod")
    # non-prod keycloak remains permissive (dev convenience)
    monkeypatch.delenv("KEYCLOAK_AUDIENCE", raising=False)
