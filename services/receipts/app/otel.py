"""Gov-enclave OpenTelemetry helper — vendored mirror of
meridian-compliance-suite packages/py/meridian_py/otel.py.

Contract: otel-foundation/DESIGN-CONTRACT.md. Env vars, attribute names
(tenant.id), prod-without-endpoint loud warning and the fail-soft rule
match the Go packages/otelx and core meridian_py.otel exactly. Gov has no
shared python package, so each service vendors this module unchanged.
...
log = logging.getLogger("gov.otel")
...
def _tenant_from_headers(headers: Any) -> str:
    ...
    (unverified) tenant_id JWT claim. Telemetry labelling only — never
    an authz decision."""
...
def init_otel(app: Optional[Any] = None, *, tracer_provider=None) -> bool:
    ...
    try:
        ...
        # Client instrumentation (idempotent; covers einvoicing/whatsapp/
        # receipts outbound flows which all ride httpx or requests).
        try:
