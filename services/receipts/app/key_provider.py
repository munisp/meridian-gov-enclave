"""KEY_PROVIDER-driven signing for the receipts service (Python mirror of
packages/keyx/provider; same env contract + fail-closed semantics).

Modes (KEY_PROVIDER env):
  software   [REAL] default — the service's existing Signer
             (RECEIPTS_SIGNING_KEY_PEM, or dev-ephemeral). Keys never leave host.
...
def build_signer(software_factory, env: dict | None = None):
    """software mode -> software_factory(); cloud-kms -> KMSSigner;
    anything else fail-closed."""
    mode = provider_mode(env)
    if mode in ("", "software"):
        return software_factory()
    if mode == "cloud-kms":
        return KMSSigner(env)
    raise KeyProviderUnavailable(
