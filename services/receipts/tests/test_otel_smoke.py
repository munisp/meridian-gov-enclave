...
os.environ["RECEIPTS_WORM_ROOT"] = "/tmp/receipts-otel-test-worm"
...
def test_span_has_tenant_id(monkeypatch):
    ...
    r = TestClient(app).get("/v1/receipts", headers={"X-Tenant-ID": "tenant-rc-1"})
    assert r.status_code == 200
    server = [s for s in exp.get_finished_spans() if s.kind == trace.SpanKind.SERVER]
    assert server
    assert any(s.attributes and s.attributes.get("tenant.id") == "tenant-rc-1"
               for s in server)
